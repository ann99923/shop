...
<body>
	...
	<%@include file="../include/admin/header.jsp" %>
		...
		<form id="modifyForm" method="post" action="/admin/authorModify">
			...
			<div class="form_section">
				...
				<div class="form_section_content">
					...
					<span id="warm_authorName">작가 이름을 입력 해주세요.</span>
				</div>
			</div>
			<div class="form_section">
				<div class="form_section_title">
				...
				<div class="form_section_content">
					<select class="input_block" name="nationId">
						...
						<option value="01" <c:out value=" ${authorInfo.nationId eq '국내' ?'selected':''}"/>>국내</option>
						<option value="02" <c:out value="${authorInfo.nationId eq '국외' ? 'selected':'' }" />>국외</option>
					</select>
				</div>
			</div>
			<div class="form_section">
			...
			<form id="moveForm" method="get">
				<input type="hidden" name="authorId" value='<c:out value="${authorInfo.authorId }"></c:out>'>
	...
	<script>
		...
		$("#cancelBtn").on("click", function(e){
			...
			$("input[name=authodId]").remove();
			moveForm.attr("action", "/admin/authorDetail");
			moveForm.submit();
		});
		
		/* 작가 수정 실행 버튼 작동 및 유효성 검사 */
		$("#modifyBtn").on("click", function(e){
			...
			if(!authorName){
				...
				$("#warm_authorName").css('display','none');
				nameCk = true;
			}
			if(!authorIntro){
				$("#warn_authorIntro").css('display','block');
			}else{
				$("#warm_authorIntro").css('display','none');
				introCk = true;
			}
